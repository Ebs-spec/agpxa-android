...
public class GpxExportService extends IntentService {
    ...
    public static final String BROADCAST_EXPORT_FAILED = "net.fabiszewski.agpxa.broadcast.write_failed";
    public static final String BROADCAST_EXPORT_DONE = "net.fabiszewski.agpxa.broadcast.write_ok";

    private static final String ns_gpx = "http://www.topografix.com/GPX/1/1";
    private static final String ns_agpxa = "https://github.com/bfabiszewski/agpxa-android/1";
    private static final String ns_xsi = "http://www.w3.org/2001/XMLSchema-instance";
    private static final String schemaLocation = ns_gpx + " http://www.topografix.com/GPX/1/1/gpx.xsd " +
            ns_agpxa + " https://raw.githubusercontent.com/bfabiszewski/agpxa-Ebs-spec/master/scripts/gpx_extensions1.xsd";
    ...
    private void serialize(@NonNull OutputStream stream) throws IOException {
        ...
        serializer.setPrefix("agpxa", ns_agpxa);
    ...
    private void writePositions(@NonNull XmlSerializer serializer)
        ...
        try (Cursor cursor = db.getPositions()) {
            ...
            while (cursor.moveToNext()) {
                ...
                // agpxa extensions (accuracy, speed, bearing, provider)
                serializer.startTag(null, "extensions");
                if (DbAccess.hasAccuracy(cursor)) {
                    writeTag(serializer, "accuracy", DbAccess.getAccuracy(cursor), ns_agpxa);
                }
                if (DbAccess.hasSpeed(cursor)) {
                    writeTag(serializer, "speed", DbAccess.getSpeed(cursor), ns_agpxa);
                }
                if (DbAccess.hasBearing(cursor)) {
                    writeTag(serializer, "bearing", DbAccess.getBearing(cursor), ns_agpxa);
                }
                if (DbAccess.hasProvider(cursor)) {
                    writeTag(serializer, "provider", DbAccess.getProvider(cursor), ns_agpxa);
                }
